    ...
    loadMore = () => {
        this.setState({ fetching_from_server: true }, async () => {
            try {
                const configs = {
                    method: "POST",
                    body: JSON.stringify({ lat: this.state.lat })
                };
                const response = await fetch(
                    `http://firstbankmi.com/git/restApp/getBiz.php?q=Get&offset=${this.offset}`,
                    configs
                        ...
                        <View style={styles.blogWrap}>
                            ...
                            <View style={styles.imgWrap}>
                                <Image
                                    ...
                                    resizeMode="cover"
                                    source={{
                                        uri: `https://1179658579.rsc.cdn77.org/${item.abbreviation}/${item.city_name}/${item.id}/original/${item.photo_name}.jpg`
                                    }}
                                />
                            </View>
                            ...
                            <View style={{ flexDirection: "row", justifyContent: "space-between", alignItems: "flex-start", marginTop: 5, paddingLeft: 5, paddingRight: 5, }}>
                                ...
                                <View style={{ flex: 1 / 2 }}>
                                    <Text style={{ textAlign: "right" }}>{item.address}</Text>
                                    <Text style={{ textAlign: "right" }}>Rating - {item.rating}</Text>
                                </View>
                            </View>
                        </View>
                    )}
